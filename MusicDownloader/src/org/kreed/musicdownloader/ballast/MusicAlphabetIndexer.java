...
public class MusicAlphabetIndexer {
	...
	private static final String[] ALPHABET = { " ", "A", "B", "C", "D", "E",
			...
			"S", "T", "U", "V", "W", "X", "Y", "Z" };
	/**
	 * The result of {@link android.provider.MediaStore.Audio#keyFor(String)}
	 * called on each letter of the alphabet.
	 */
	...
	public int getPositionForSection(int sectionIndex) {
		...
		Log.e("musicAlphabetIndexer", "getPositionForSection");
		if (cursor == null)
			return 0;

		if (sectionIndex <= 0) {
			Log.e("musicAlphabetIndexer",
					"getPositionForSection sectionIndex<=0");
			return 0;
		}
		int count = cursor.getCount();
		if (sectionIndex >= ALPHABET.length) {
			Log.e("musicAlphabetIndexer",
					"getPositionForSection section>=alphabet.length "
							+ ALPHABET.length + " section index "
							+ sectionIndex + " count " + count);
			return count;
		}

		int pos = mAlphaMap[sectionIndex];
		if (pos != -1) {
			Log.e("musicAlphabetIndexer", "getPositionForSection pos " + pos);
			return pos;
		}
		...
		pos = (end + start) / 2;
		while (pos < end) {
			Log.e("musicAlphabetIndexer", "getPositionForSection e in while");
			...
			if (diff != 0) {
				if (diff < 0) {
					...
					if (start >= count) {
						Log.e("musicAlphabetIndexer",
								"getPositionForSection e in while in start>count "
										+ count);
						pos = count;
						break;
					}
					...
					Log.e("musicAlphabetIndexer",
							"getPositionForSection e in while end=pos " + pos);
				}
			} else {
		...
		Log.e("musicAlphabetIndexer",
				"getPositionForSection e la final si pos este " + pos);
		return pos;
	}
	...
	public int getSectionForPosition(int position) {
		...
		String key = MediaStore.Audio.keyFor(mDataCursor
				.getString(mColumnIndex));
		mDataCursor.moveToPosition(savedCursorPos);

		String[] alphabet = ALPHABET_KEYS;
		for (int i = 1, len = alphabet.length; i != len; ++i) {
			Log.e("musicAlphabetIndexer", "getSectionForPosition e in for ");
			if (key.startsWith(alphabet[i])) {
				Log.e("musicAlphabetIndexer",
						"getSectionForPosition e in for si in if");
				return i;
			}
		}
