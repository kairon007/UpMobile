...
public class SwipeUndoTouchListener extends SwipeDismissTouchListener {
    ...
    protected void afterViewFling(@NonNull final View view, final int position, Object o) {
        if (mUndoPositions.contains(position)) {
            ...
            mCallback.onUndoShown(view, position, o);
            showUndoView(view);
    ...
    protected void performDismiss(@NonNull final View view, final int position, Object o) {
        super.performDismiss(view, position, o);

        mDismissedViews.add(view);
        mDismissedPositions.add(position);
        dismissObjects.add(o);
        mCallback.onDismiss(view, position, o);
    }
    ...
    public void dimissPending() {
        for (int position : mUndoPositions) {
            performDismiss(mUndoViews.get(position), position, getListViewWrapper().getAdapter().getItem(position));
        }
    }
    ...
    protected void finalizeDismiss() {
        if (getActiveDismissCount() == 0 && getActiveSwipeCount() == 0) {
            ...
            android.util.Log.d("logd", "finalizeDismiss: +++++++++++++++++++++++++++");
            for (Object o : dismissObjects) {
            	android.util.Log.d("logd", "finalizeDismiss: 1" + o);
            }
            for(int i : mDismissedPositions) {
                android.util.Log.d("logd", "finalizeDismiss: 2" + i);
            }
            android.util.Log.d("logd", "finalizeDismiss: ++++++++++++++++++++++++++++");
            notifyCallback(mDismissedPositions, dismissObjects);
     ...
     * @param view the parent {@code View} which contains both primary and undo {@code View}s.
     */
    public void undo(@NonNull final View view, Object o) {
        ...
        mCallback.onUndo(view, position, o);
    }
