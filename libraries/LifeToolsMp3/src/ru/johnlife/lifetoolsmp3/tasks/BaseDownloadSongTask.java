...
//	protected Bitmap cover;
    ...
    protected boolean isEarlierDownloaded = false;
    ...
    public BaseDownloadSongTask(final Context context, RemoteSong song, int id, boolean earlierMessage) {
        ...
        isEarlierDownloaded = StateKeeper.DOWNLOADED == StateKeeper.getInstance().checkSongInfo(downloadingSong.getComment());
        earlierMsg = isEarlierDownloaded ? false :true;
        songId = downloadingSong instanceof GrooveSong ? ((GrooveSong) downloadingSong).getSongId() : -1;
        headers = downloadingSong.getHeaders();
    ...
    public void downloadSong(boolean fromCallback) {
        ...
        if (!isEarlierDownloaded) StateKeeper.getInstance().putSongInfo(downloadingSong.getComment(), AbstractSong.EMPTY_PATH, StateKeeper.DOWNLOADING);
...
//			request.setNotificationVisibility(DownloadManager.Request.VISIBILITY_VISIBLE | DownloadManager.Request.VISIBILITY_VISIBLE_NOTIFY_COMPLETED);
    ...
    protected void notifyAboutFailed(long downloadId, final RemoteSong s) {
        if (!isEarlierDownloaded) StateKeeper.getInstance().removeSongInfo(s.getComment());
        ((Activity) context).runOnUiThread(new Runnable() {
