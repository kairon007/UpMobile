		...
		try {
			...
			if(result != null) {
			    for(Track track : result) {
						...
						addSong(new SoundCloudV2Song(downloadUrl, largeCoverUrl.equals("null") ? "NOT_FOUND" : largeCoverUrl).setArtistName(songArtist).setDuration(duration).setTitle(songTitle));
			    }
			}
		} catch (Exception e) {
			e.printStackTrace();
