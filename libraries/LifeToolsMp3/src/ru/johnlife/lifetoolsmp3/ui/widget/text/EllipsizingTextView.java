		...
		if (maxLines != -1) {
			...
			if (originalLineCount > maxLines) {
				if (this.getEllipsize() == TruncateAt.START) {
					workingText = fullText.substring(
							layout.getLineStart(originalLineCount - maxLines
									- 1)).trim();
					while (createWorkingLayout(ELLIPSIS + workingText)
					...
					int secondOffset = layout.getLineEnd(originalLineCount - 1)
							- firstOffset + 1;
					String firstWorkingText = fullText
							.substring(0, firstOffset).trim();
					String secondWorkingText = fullText.substring(secondOffset)
							.trim();
					while (createWorkingLayout(
							firstWorkingText + ELLIPSIS + secondWorkingText)
							.getLineCount() > maxLines) {
						if (shrinkLeft) {
							shrinkLeft = false;
					...
					workingText = firstWorkingText + ELLIPSIS
							+ secondWorkingText;
				}
				ellipsized = true;
			}
