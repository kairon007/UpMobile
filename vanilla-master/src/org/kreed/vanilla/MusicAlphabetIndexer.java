...
public class MusicAlphabetIndexer {
	...
	{ Log.e("musicAlphabetIndexer","MusicAlphabetIndexer constructor");
	...
	{Log.e("musicAlphabetIndexer","getSections");
		return ALPHABET;
	}
	...
	{Log.e("musicAlphabetIndexer","setCursor");
		mDataCursor = cursor;
	...
	public int getPositionForSection(int sectionIndex) {
		Cursor cursor = mDataCursor; Log.e("musicAlphabetIndexer","getPositionForSection");
		if (cursor == null)
			return 0;

		if (sectionIndex <= 0){Log.e("musicAlphabetIndexer","getPositionForSection sectionIndex<=0");
			return 0;}
		int count = cursor.getCount();
		if (sectionIndex >= ALPHABET.length){Log.e("musicAlphabetIndexer","getPositionForSection section>=alphabet.length "+ALPHABET.length + " section index "+ sectionIndex + " count "+count);
			return count;}

		int pos = mAlphaMap[sectionIndex];
		if (pos != -1){Log.e("musicAlphabetIndexer","getPositionForSection pos "+pos);
			return pos;}
		...
		pos = (end + start) / 2;
		while (pos < end) {Log.e("musicAlphabetIndexer","getPositionForSection e in while");
			cursor.moveToPosition(pos);
			String curName = cursor.getString(mColumnIndex);
			String curKey    = MediaStore.Audio.keyFor(curName);

			String curLetter = ( curKey.length() >= 3 ? curKey.substring(0, 3) : "\t~\t");
			int diff = curLetter.compareTo(targetLetter);
			if (diff != 0) {
				if (diff < 0) {
					start = pos + 1;
					if (start >= count) {Log.e("musicAlphabetIndexer","getPositionForSection e in while in start>count "+ count);
						pos = count;
						break;
					}
				} else {
					end = pos; Log.e("musicAlphabetIndexer","getPositionForSection e in while end=pos " + pos);
				}
			} else {
		...
		cursor.moveToPosition(savedCursorPos); Log.e("musicAlphabetIndexer","getPositionForSection e la final si pos este " + pos);
		return pos;
	}
	...
	{Log.e("musicAlphabetIndexer","getSectionForPosition");
		...
		for (int i = 1, len = alphabet.length; i != len; ++i) {Log.e("musicAlphabetIndexer","getSectionForPosition e in for ");
			if (key.startsWith(alphabet[i])){Log.e("musicAlphabetIndexer","getSectionForPosition e in for si in if");
				return i;}
		}
